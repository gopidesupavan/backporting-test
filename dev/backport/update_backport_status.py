...
def add_comments(backport_url: str, target_branch: str, commit_sha: str, source_pr_number: str):
    ...
    if backport_url.strip() != "EMPTY":
        ...
        comment = get_success_comment(branch=target_branch, pr_url=backport_url,
                                      pr_number=pr_number)
    else:
        commit_sha_url = f"https://github.com/{os.getenv('REPOSITORY')}/commit/{commit_sha}"
        comment = get_failure_comment(branch=target_branch, commit_sha_url=commit_sha_url, commit_sha=commit_sha)
